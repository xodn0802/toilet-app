...
- 완료(코드) P0 7번 로그인 — 카카오 소셜 로그인. **대시보드 설정이 남아 있어
  아직 동작하지 않는다.** 아래 "카카오 로그인" 절의 체크리스트를 먼저 끝낼 것.
...
으로 가는 것까지 확인했다. **실제 카카오 왕복은 자동화하지 않았다** — 카카오
계정 로그인이 필요해 사람이 해야 한다. 닉네임 표시·세션 유지·로그아웃은 설정이
끝난 뒤 사람이 한 번 밟아야 검증된다.

### 다음 할 일 (2026-08-01 기준)

**지금 막고 있는 것은 코드가 아니라 대시보드 설정이다.** 위 "카카오 로그인" 절의
체크리스트를 끝내고, 로컬에서 실제 카카오 왕복을 한 번 밟은 뒤 배포한다.
설정 전에 배포하면 프로덕션 로그인 버튼이 날 JSON 을 뱉는다.
...
1. **P0 7번 로그인 마무리** — 대시보드 설정 + 실제 왕복 확인 + 배포.

...
- 완료 반경 표시 — 앱바가 `1km 이내 5곳`처럼 기준과 함께 말하고, 반경 밖 마커는
  흐려진다. 아래 "반경 표시" 절 참고.
...
검증을 하려면 휠로 두 단계 줄여야 `boundingBox()`가 잡힌다.

### 다음 할 일

네 건 중 **배포 지도 미표시·위치 정확도·반경 표시가 끝났다.**

**다크 모드 토글은 사용자가 미뤘다(2026-08-01).** 조사는 끝나 있으니 다시 할
때는 위 절의 `light-dark()` 부분부터 읽으면 된다. 그러니 다음은 **로그인**이다.
